...
You will find a sensor called mmWave (in the image below it's called "Living Room mmWave") - this is the output from the mmWave sensor directly and indicates if movement is detected. The mmWave sensor has a configurable offset for the "Blind time" which we will cover below. The default blind time is 12.5 seconds, which is basically how long the sensor takes to go to an "Off" or "clear" state after motion has stopped being detected:

Image
{: .label .label-red }

The PIR sensor ("Living Room PIR" in the below image) is the reading from the PIR sensor on whether motion was detected or not. It also has a user configurable timeout period, which by default is 10 seconds. This means that it will take 10 seconds after the last motion for the PIR sensor to go back to the "Off" or "Clear" state:
...
The mmWave LED disables the LED on the mmWave sensor itself which flashes when in use:

Image
{: .label .label-red }

The ESP32 Status LED disables the LED on the EP1 board:
